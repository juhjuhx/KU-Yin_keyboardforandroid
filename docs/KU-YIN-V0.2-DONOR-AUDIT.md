...
## M3 candidate-presentation provenance (2026-09-14)

M3 donor inspection (no file content copied anywhere):

- fcitx5-android (`fcitx5-android/fcitx5-android`, master, LGPL-2.1 verified):
  horizontal strip (`.../input/candidates/horizontal/HorizontalCandidateComponent.kt`,
  `.../horizontal/HorizontalCandidateViewAdapter.kt`) + expanded grid/flexbox
  windows (`.../candidates/expanded/window/BaseExpandedCandidateWindow.kt`,
  `FlexboxExpandedCandidateWindow.kt`, `GridExpandedCandidateWindow.kt`) sharing
  decoder-ordered data, expand affordance as explicit state
  (`ExpandButtonStateMachine`). Verdict: GREEN for concepts.
- fcitx5-chewing plugin tree (same repo): select-by-index into decoder snapshot;
  matches KU-Yin `selectCandidateUpdate -> applyEngineUpdate`. GREEN concepts.
- FlorisBoard (Apache-2.0 LICENSE verified): architecture reference only, no
  source opened for M3. GREEN, unused.
- HeliBoard (`Helium314/HeliBoard`, GPL-3.0 LICENSE verified): reference-only,
  no source opened, nothing copied. RED for copying.
- Gboard: visual-only, never copy.

M3 implementation (`CandidateState`, `candidateStateOf`,
`SelectCandidate` command/effect, custom `CandidateView` collapsed/expanded
rendering, engine `canPage*` queries) is original KU-Yin code against spec
section 14 and the existing controller/effect bridge. Zero donor files copied,
zero new dependencies (deliberately no RecyclerView/Flexbox/paging libs).
